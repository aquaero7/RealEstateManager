...
object ContentProviderTestUtils {
    ...
    val uriProperties = AppContentProvider.URI_PROPERTIES
    val uriPropertiesForSale = AppContentProvider.URI_PROPERTIES_FOR_SALE
    val uriSoldProperties = AppContentProvider.URI_SOLD_PROPERTIES
    private val projectionProperty = arrayOf(PropertyKey.PROPERTY_ID, PropertyKey.SALE_DATE)
    val uriAddresses = AppContentProvider.URI_ADDRESSES
    private val projectionAddress = arrayOf(AddressKey.ADDRESS_ID)
    val uriPhotos = AppContentProvider.URI_PHOTOS
    private val projectionPhoto = arrayOf(PhotoKey.PHOTO_ID)
    val uriAgents = AppContentProvider.URI_AGENTS
    private val projectionAgent = arrayOf(AgentKey.AGENT_ID)
    val uriTypes = AppContentProvider.URI_TYPES
    private val projectionType = arrayOf(TypeKey.TYPE_ID)
    val uriPois = AppContentProvider.URI_POIS
    private val projectionPoi = arrayOf(PoiKey.POI_ID)
    val uriPropertyPoiJoins = AppContentProvider.URI_PROPERTY_POI_JOINS
    ...
    val projections = listOf(
        projectionProperty, projectionAddress, projectionPhoto,
        projectionAgent, projectionType, projectionPoi, projectionPropertyPoiJoin
    )

    fun cursorFields(uri: Uri, cursor: Cursor): Pair<Any?, String?> {
        var first: Any? = null
        ...
        when (uri) {
            ...
            uriTypes -> {
                first = cursor.getString(cursor.getColumnIndex(TypeKey.TYPE_ID))
            }

            uriPois -> {
                first = cursor.getString(cursor.getColumnIndex(PoiKey.POI_ID))
            }
    ...
    fun initTest(
        ...
        uri: Uri
    ): Pair<Cursor?, Triple<String?, String?, String?>> {
        ...
        var strings: Triple<String?, String?, String?> =
        ...
        runBlocking {
            ...
            when (uri) {
                ...
                uriPropertyPoiJoins -> {
                    ...
                    populateDatabaseWithPropertyPoiJoin(
                        properties = propertiesPPJ,
                        propertyPoiJoin = ppj1
                    )
                    populateDatabaseWithPropertyPoiJoin(
                        properties = propertiesPPJ,
                        propertyPoiJoin = ppj2
                    )
                    populateDatabaseWithPropertyPoiJoin(
                        properties = propertiesPPJ,
                        propertyPoiJoin = ppj3
                    )
                    populateDatabaseWithPropertyPoiJoin(
                        properties = propertiesPPJ,
                        propertyPoiJoin = ppj4
                    )
                    projection = projectionPropertyPoiJoin
                }
            }
            cursor = cursorAndQuery(
                contentResolver = contentResolver,
                uri = uri,
    ...
    fun initDatabase(context: Context) {
        val testDatabase = AppDatabase.getInstance(context = context)
